...
const MenuPatient = props => {
   ...
   return (
      <Menu id='simple-menu' anchorEl={anchor} elevation={1} keepMounted open={true} onClose={close}>
...
const PopoverPatient = props => {
   ...
   function handleSubmitOutcome(event) {
      ...
      dispatch(updatePatient({ ...patient, outcome, outcomeDate, comorbidities, hospitalizationReason }))
      setShow(false)
   }
   ...
   return (
      <Popover open={show} anchorEl={anchor} onClose={close}>
         <form className={classes.formOutcome} onSubmit={handleSubmitOutcome}>
            {patient.name}
            <RadioGroup aria-label='outcome' name='outcome' value={outcome} onChange={e => setOutcome(e.target.value)}>
               <FormControlLabel value='discharge' control={<Radio />} label='Alta' />
               <FormControlLabel value='death' control={<Radio />} label='Óbito' />
            </RadioGroup>
            <Button className={classes.buttonOutcome} type='submit' variant='outlined' color='primary'>
               Enviar
            </Button>
         </form>
      </Popover>
...
const WarningDialog = props => {
   ...
   return (
      ...
      >
         <DialogTitle id='alert-dialog-slide-title'>{'Todos os leitos estão ocupados'}</DialogTitle>
         <DialogContent>
...
const Beds = props => {
   ...
   return (
      <Layout>
         <CssBaseline />
         {anchorEl && patient && (
            <MenuPatient anchor={anchorEl} patient={patient} close={handleClose} setDeleteDialog={setDeleteDialog} />
         )}
         ...
         <Grid container component='main' spacing={2}>
            {patients ? (
                     ...
                     {patient.outcome === 'pending' && (
                        <Grid item xs={12} sm={6} md={4}>
                           <Card className={classes.card}>
                              <CardHeader
                                 avatar={<Avatar aria-label='recipe'>{patient.bed}</Avatar>}
                                 action={
                                    <React.Fragment>
                                       <IconButton
                                 ...
                                 title={patient.name}
                                 subheader={patient.birthday && `${age(patient.birthday)} anos`}
                              />
                              <CardContent className={classes.cardContent}>
                                 {patient.comorbidities && (
                                    <Typography variant='body2' color='textSecondary' component='p'>
                                       {patient.comorbidities.length > 1 ? <b>Comorbidades:</b> : <b>Comorbidade:</b>}{' '}
                                       {patient.comorbidities.map((c, index) => (
                                          <span key={index}>
                                             {c.name}
                                             {index !== patient.comorbidities.length - 1 ? ', ' : '.'}
                                          </span>
                                       ))}
                                    </Typography>
                                 )}
                                 {patient.hospitalizationReason && (
                                    <Typography variant='body2' color='textSecondary' component='p'>
                                       {patient.hospitalizationReason.length > 1 ? (
                                          <b>Motivos da internação:</b>
                                       ) : (
                                          <b>Motivo da internação:</b>
                                       )}{' '}
                                       {patient.hospitalizationReason.map((h, index) => (
                                          <span key={index}>
                                             {h.name}
                                             {index !== patient.hospitalizationReason.length - 1 ? ', ' : '.'}
                                          </span>
                                       ))}
                                    </Typography>
                                 )}
               ...
               <Typography variant='h5' component='span'>
                  Não há pacientes cadastrados! Adicione um paciente clicando em (+) no canto inferior direito.
               </Typography>
            )}
         </Grid>
         <Fab className={classes.fab} color='primary' aria-label='add' onClick={handleClickAddPatient}>
            <Add />
         </Fab>

         {warningDialog && <WarningDialog warningDialog={warningDialog} setWarningDialog={setWarningDialog} />}

         {deleteDialog && (
            <DeleteDialog deleteDialog={deleteDialog} setDeleteDialog={setDeleteDialog} patientId={patient.id} />
         )}
      </Layout>
   )
}
